...
class QRSDetectorOffline(object):
    ...
    def __init__(self, ecg_data_path, verbose=True, log_data=False, plot_data=False, show_plot=False):
        ...
        self.filter_lowcut = 0.0 + sys.float_info.epsilon
        ...
        self.detect_qrs()
        self.rr_intervals()
        for k, v in self.timedomain().items():
            print("- %s: %.2f" % (k, v))
    ...
    def detect_peaks(self):
        ...
        ecg_measurements = self.ecg_data_raw[0:5001, 1]
    ...
    def rr_intervals(self):
        self.rr=np.diff(self.qrs_peaks_indices)
        self.rr=self.rr*2
    ...
    def timedomain(self):
        ...
        results['Mean RR (ms)'] = np.mean(self.rr)
        results['STD RR/SDNN (ms)'] = np.std(self.rr)
        results['Mean HR (Kubios\' style) (beats/min)'] = 60000/np.mean(self.rr)
        results['Mean HR (beats/min)'] = np.mean(hr)
        results['STD HR (beats/min)'] = np.std(hr)
        results['Min HR (beats/min)'] = np.min(hr)
        results['Max HR (beats/min)'] = np.max(hr)
        results['RMSSD (ms)'] = np.sqrt(np.mean(np.square(np.diff(self.rr))))
        results['NNxx'] = np.sum(np.abs(np.diff(self.rr)) > 50)*1
        results['pNNxx (%)'] = 100 * np.sum((np.abs(np.diff(self.rr)) > 50)*1) / len(self.rr)
        return results
    ...
    def plot_detection_data(self, show_plot=False):
        ...
        plot_data(axis=axarr[0], data=self.ecg_data_raw[0:5001, 1], title='Raw ECG measurements')
        ...
        plot_data(axis=axarr[5], data=self.ecg_data_detected[0:5001, 1], title='Raw ECG measurements with QRS peaks marked (black)')
        plot_points(axis=axarr[5], values=self.ecg_data_detected[0:5001, 1], indices=self.qrs_peaks_indices)
...
if __name__ == "__main__":
    qrs_detector = QRSDetectorOffline(ecg_data_path="ecg_data/ecg.csv", verbose=True,
